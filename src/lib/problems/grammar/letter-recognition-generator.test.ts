...
describe('LetterRecognitionGenerator', () => {
	...
	describe('generate', () => {
		it('should generate a valid letter recognition problem', () => {
			...
			expect(result!.signature).toMatch(/^letter-recognition:d1:[A-Z]:lower$/)
		})
	...
	describe('allPossibleSignatures', () => {
		...
		it('should only have lowercase signatures at all difficulties', () => {
			for (const difficulty of [1, 2, 3, 4] as DifficultyLevel[]) {
				const signatures = generator.allPossibleSignatures(difficulty)
				const allLowercase = signatures.every((s) => s.endsWith(':lower'))
