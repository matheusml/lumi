...
export class ComparisonProblemGenerator implements ProblemGenerator {
	...
	private params(difficulty: DifficultyLevel): { range: [number, number]; minDiff: number } {
		switch (difficulty) {
			case 1: return { range: [1, 10], minDiff: 4 };  // Large obvious differences
			case 2: return { range: [1, 15], minDiff: 3 };  // Medium differences
			case 3: return { range: [1, 20], minDiff: 2 };  // Smaller differences
			case 4: return { range: [1, 20], minDiff: 1 };  // Can be very close
		}
	}

	generate(difficulty: DifficultyLevel, excluding: Set<string>): GeneratorResult | null {
		const { range: [min, max], minDiff } = this.params(difficulty);
